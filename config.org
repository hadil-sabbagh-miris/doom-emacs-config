...
(use-package! claude-code-ide
  ...
  ;; Mouse wheel in Claude buffers: with `no-flicker' on (= Claude Code
  ;; fullscreen rendering), vterm only holds the current viewport and
  ;; doesn't forward mouse-wheel escape sequences to the pty. Route wheel
  ;; events to Meta+k / Meta+j, which ~/.claude/keybindings.json binds to
  ;; `scroll:lineUp' / `scroll:lineDown' in the Scroll context — gives a
  ;; line-at-a-time feel instead of PgUp/PgDn's half-page jumps.
  (defun my/claude-code-ide-wheel-up (_e)
    (interactive "e")
    (vterm-send-key "k" nil t nil))   ; Meta+k → scroll:lineUp
  (defun my/claude-code-ide-wheel-down (_e)
    (interactive "e")
    (vterm-send-key "j" nil t nil))   ; Meta+j → scroll:lineDown
  (advice-add
   'claude-code-ide--setup-terminal-keybindings :after
   (defun my/claude-code-ide-bind-wheel ()
     ;; Only needed when no-flicker is on — Claude manages its own
     ;; transcript so native Emacs scroll is inert. With no-flicker off,
     ;; vterm holds real scrollback and native wheel scroll just works.
     (when (and claude-code-ide-no-flicker
       ...
       (dolist (ev '([wheel-up] [double-wheel-up] [triple-wheel-up]))
         (local-set-key ev #'my/claude-code-ide-wheel-up))
       (dolist (ev '([wheel-down] [double-wheel-down] [triple-wheel-down]))
         (local-set-key ev #'my/claude-code-ide-wheel-down))))))
#+end_src
